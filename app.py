...
# Opcional: desactivar overhead de seguimiento de objetos
...
class Ruta(db.Model):
    ...
    ip = db.Column(db.String(45), nullable=False)  # IPv6 hasta 45 chars
...
# Crea las tablas (en producción usa migraciones)
...
def send_data():
    try:
        ...
        db.session.commit()
        logging.info("Datos guardados en la tabla 'ruta' con id %s", nueva_ruta.id)

        # (Opcional) También escribir en log.html
        html_data = (
            f"<pre>"
    ...
    try:
        # --- Mismo bloque de envío de correo que ya tenías ---
